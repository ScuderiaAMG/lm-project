...
def load_and_preprocess_data():
    ...
    try:
        ...
        logger.info(f"✅ Dataset loaded successfully from cache!")
        logger.info(f"📈 Available splits: {list(dataset.keys())}")
        logger.info(f"🏋️ Train samples: {len(dataset['train']):,}")
        logger.info(f"🔍 Validation samples: {len(dataset['validation']):,}")
        
        # 从现有模型加载tokenizer
        logger.info("🔤 Loading tokenizer from existing model...")
        existing_model_dir = "./models/final_model_20260115_044955"
        
        if not os.path.exists(existing_model_dir):
            logger.warning("❌ Existing model directory not found, creating new tokenizer...")
        ...
        tokenized_datasets = dataset.map(
            ...
            cache_file_names={
                "train": "./data/train_wikitext103_1024.cache",
                "validation": "./data/validation_wikitext103_1024.cache"
            }
        )
...
def load_wikitext2_dataset():
    """回退到WikiText-2数据集"""
    logger.warning("⚠️ Using WikiText-2 dataset as fallback")
    ...
    existing_model_dir = "./models/final_model_20260115_044955"
    tokenizer = GPT2Tokenizer.from_pretrained(
        existing_model_dir if os.path.exists(existing_model_dir) else "gpt2",
        local_files_only=True
    )
    tokenizer.pad_token = tokenizer.eos_token
...
def create_model_from_existing():
    """从现有124M模型创建GPT-2 Medium模型"""
    logger.info("🏗️ Creating GPT-2 Medium model (355M parameters) from existing weights...")
    ...
    try:
        ...
        if os.path.exists(existing_model_dir):
            logger.info("✅ Loading base weights from existing model...")
            base_model = GPT2LMHeadModel.from_pretrained(
                existing_model_dir,
                local_files_only=True
            )
        else:
            logger.info("🔄 Creating new GPT-2 base model...")
        ...
        # 创建新的GPT-2 Medium模型
        logger.info("🏗️ Creating GPT-2 Medium architecture...")
        ...
        for key in base_state_dict.keys():
            if key in medium_state_dict and base_state_dict[key].shape == medium_state_dict[key].shape:
                medium_state_dict[key] = base_state_dict[key]
                transferred += 1
        
        medium_model.load_state_dict(medium_state_dict)
        logger.info(f"✅ Successfully transferred {transferred} weight parameters")
        
        # 应用8-bit量化
        logger.info("⚡ Applying 8-bit quantization...")
        medium_model = medium_model.to("cuda")
        
        # 启用梯度检查点
        if hasattr(medium_model, "gradient_checkpointing_enable"):
            medium_model.gradient_checkpointing_enable()
            logger.info("✅ Gradient checkpointing enabled")
        
        # 打印模型信息
        total_params = medium_model.num_parameters()
        trainable_params = sum(p.numel() for p in medium_model.parameters() if p.requires_grad)
        ...
        logger.info("🔄 Falling back to standard GPT-2 Medium...")
        return GPT2LMHeadModel.from_pretrained("gpt2-medium").cuda()

def main():
    """主训练函数"""
    console.rule("[bold blue]🚀 GPT-2 Medium Training (355M Parameters) - Offline Mode")
    ...
    try:
        ...
        # 创建模型
        model = create_model_from_existing()
        model.to(device)
        ...
        training_args = TrainingArguments(
            ...
            num_train_epochs=5,    # 5个epoch（可调整）
        ...
        # 保存最终模型
        final_model_dir = f"./models/final_model_medium_offline_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def generate_example_text(model, tokenizer, device, output_dir):
    ...
    try:
        ...
        logger.warning(f"⚠️ Failed to generate example text: {str(e)}")
